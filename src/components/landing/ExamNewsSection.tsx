  ...
  const getStatusBadge = (status: ExamNewsItem["status"]) => {
    switch (status) {
      case "Registration Open":
        return <Badge className="bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/25 border-emerald-500/30">● {status}</Badge>;
      case "Admit Card Released":
        return <Badge className="bg-amber-500/15 text-amber-600 dark:text-amber-400 hover:bg-amber-500/25 border-amber-500/30">● {status}</Badge>;
      case "Results Declared":
        return <Badge className="bg-blue-500/15 text-blue-600 dark:text-blue-400 hover:bg-blue-500/25 border-blue-500/30">● {status}</Badge>;
      default:
    ...
    <section id="exam-news" className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <Button
          ...
          className="h-10 rounded-xl gap-2 border-primary/20 hover:bg-primary/5 self-start md:self-auto"
        >
      ...
      <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:w-64 shrink-0">
          ...
          <Input
            ...
            className="pl-9 h-10 rounded-xl bg-card border-border/80 text-sm"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <Card key={i} className="animate-pulse rounded-2xl p-6 space-y-4">
              <div className="h-5 bg-accent/60 rounded w-1/3" />
              <div className="h-6 bg-accent/80 rounded w-3/4" />
              <div className="h-16 bg-accent/50 rounded w-full" />
              <div className="h-4 bg-accent/40 rounded w-1/2" />
            </Card>
          ))}
        </div>
      ) : filteredNews.length === 0 ? (
        <div className="mt-12 text-center py-12 rounded-2xl bg-card/50 border border-dashed border-border">
          <Bell className="mx-auto size-10 text-muted-foreground/60" />
          <h3 className="mt-3 text-lg font-medium">No exam news found</h3>
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredNews.map((item) => (
            <Card
              key={item.id}
              className="hover-lift glass group flex flex-col justify-between rounded-2xl p-6 border-border/70 transition-all hover:border-primary/40"
            >
              <div>
                <div className="flex items-center justify-between gap-2">
                  <Badge variant="outline" className="text-[11px] font-medium border-primary/20 bg-primary/5">
                    {item.category}
                  </Badge>
                  {getStatusBadge(item.status)}
                </div>

                <h3 className="mt-4 font-display text-lg font-bold text-foreground group-hover:text-primary transition-colors line-clamp-2">
                  {item.title}
                </h3>
              ...
              <div className="mt-5 flex items-center justify-between pt-2">
                <Button
                  ...
                  className="h-9 px-3 text-xs font-semibold text-primary hover:text-primary hover:bg-primary/10 rounded-xl"
                >
        ...
        <Dialog open={!!selectedExam} onOpenChange={() => setSelectedExam(null)}>
          <DialogContent className="max-w-xl rounded-3xl p-6 sm:p-8">
            ...
            <div className="mt-4 space-y-4 text-sm">
              <div className="rounded-xl bg-accent/50 p-4 border border-border/60">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Eligibility Criteria</p>
                <p className="mt-1 font-medium text-foreground">{selectedExam.eligibility}</p>
              </div>

              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Overview</p>
                <p className="mt-1 leading-relaxed text-muted-foreground">{selectedExam.summary}</p>
              </div>

              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Key Highlights & Updates</p>
              ...
              <div className="pt-4 border-t border-border flex items-center justify-between">
                ...
                {selectedExam.officialUrl && (
                  <Button className="gradient-brand text-primary-foreground rounded-xl text-xs gap-1.5" asChild>
